...
func (ctl *LianghuaController) get_order(ctx *gin.Context) {
	...
	ctx.JSON(200, gin.H{"code": 1, "data": order})
}
...
func (ctl *LianghuaController) edit(ctx *gin.Context) {
	...
	for i := 0; i < len(oldCtxts); i++ {
		...
		if oldCtxts[i].SellOrder != "" {
			oldHuobi.SubmitCancel(oldCtxts[i].SellOrder)
			newHuobi.SubmitCancel(oldCtxts[i].SellOrder)
		}
	}
	...
	//启动服务
	if params.Run == 1 {
		go robot.NewGridBuy(params.Id, params.Model, params.AccountId, params.BaseCurrency, params.QuoteCurrency, params.Datas, params.Owner, params.AccessKey, params.SecretKey).Start()
	}
...
func (ctl *LianghuaController) GetTradeDetail(ctx *gin.Context) {
	huobi := models.NewHuoBiEx("", "")
	ret := huobi.GetTradeDetail("ethusdt")
	ctx.JSON(200, ret)
}
...
func (ctl *LianghuaController) get_accounts(ctx *gin.Context) {
	...
	huobi := models.NewHuoBiEx(params.AccessKey, params.SecretKey)
	accounts := huobi.GetAccounts()
	log.Println(accounts)
	if accounts.Status != "ok" {
		ctx.JSON(200, gin.H{"code": 1, "message": accounts.ErrMsg})
		return
	}
	ctx.JSON(200, gin.H{"code": 0, "data": accounts.Data})
	return
}

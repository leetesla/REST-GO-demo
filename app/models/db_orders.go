...
func (m *Orders) Flush(stracyid int) error {
	...
	for i := 0; i < len(infos); i++ {
		order := huobi.GetOrder(fmt.Sprint(infos[i].OrderId))
		//log.Println(order)
		...
		if order.Data.Finishedat > 0 {
			...
			infos[i].Fieldamount, _ = strconv.ParseFloat(order.Data.Fieldamount, 64)
			infos[i].Fieldcashamount, _ = strconv.ParseFloat(order.Data.Fieldcashamount, 64)
			fee, _ := strconv.ParseFloat(order.Data.Fieldfees, 64)
